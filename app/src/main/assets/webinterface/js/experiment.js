...
var blockEditor = {
    props: ['currentBlock'],
    template: '<div class="two-element-container"><select class="select" v-model="currentBlock.friendly_name"><option value="Animation">Animation</option><option value="Display">Display</option><option value="Reset Display">Reset Display</option><option value="Look At Target">Look At Target</option><option value="Reset Look At">Reset Look At</option><option value="Move To">Move To</option><option value="Say">Say</option><option value="Sound">Sound</option><option value="Time">Time</option><option value="Empty">Empty</option></select><div></div><input type="checkbox" class="checkbox" v-model="currentBlock.stopping"><input type="checkbox" class="checkbox" v-model="currentBlock.requires_user_interaction"></div>'
}
